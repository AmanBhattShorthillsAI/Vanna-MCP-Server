...
# --- VANNA DDL, DOCUMENTATION, AND TRAINING DATA (Unchanged) ---
ddl_statements = [
  ...
  `k_symbol` TEXT
    ...
    {"description": """CREATE TABLE `trans` (
  `trans_id` INT,
  `account_id` INT,
  `date` DATE,
  `type` TEXT,
...
documentation_entries = [
    {"description": """Table `account`: Contains information about each account, including its creation date, location, and frequency of statement issuance. This table holds the primary details for each bank account.
Columns:
...
- `date`: The creation date of the account in YYMMDD format."""},
    {"description": """Table `card`: Details about credit cards issued to clients. This table lists all credit cards associated with dispositions.
Columns:
...
- `issued`: The date when the credit card was issued in YYMMDD format."""},
    {"description": """Table `client`: Contains personal demographic information about the bank's clients.
Columns:
...
- `district_id`: The ID for the location of the client's branch, foreign key to the district table."""},
    {"description": """Table `disp`: Links clients to their accounts and specifies their rights. This table acts as a mapping between clients and accounts.
Columns:
...
- `type`: The type of disposition or role of the client for that account. Values are: 'OWNER' (the account owner), 'DISPONENT' (a user with rights to the account)."""},
    {"description": """Table `district`: Provides demographic and economic statistics for different geographic districts.
Columns:
...
- `A16`: The number of committed crimes in 1996."""},
    {"description": """Table `loan`: Contains information about approved loans for accounts.
Columns:
...
- `status`: The repayment status of the loan. Values are: 'A' (contract finished, no problems), 'B' (contract finished, loan not paid), 'C' (running contract, OK so far), 'D' (running contract, client in debt)."""},
    {"description": """Table `order`: Contains information about permanent (standing) orders from accounts.
Columns:
...
- `k_symbol`: A characterization of the payment's purpose. Values are: 'POJISTNE' (insurance payment), 'SIPO' (household payment), 'LEASING' (leasing payment), 'UVER' (loan payment)."""},
    {"description": """Table `trans`: Records all transactions for accounts, providing a detailed log.
Columns:
...
def train_vanna(vn):
    ...
    """
    logging.info("--- Training on DDL statements ---")
    for item in ddl_statements:
        vn.train(ddl=item["description"])
    logging.info("DDL training complete.")

    logging.info("--- Training on Documentation ---")
    for item in documentation_entries:
        vn.train(documentation=item["description"])
    logging.info("Documentation training complete.")

    logging.info("--- Training on Question-SQL pairs ---")
    for pair in training_data:
        ingested_id = vn.train(question=pair["question"], sql=pair["sql"])
        logging.info(f"  - Ingested Q: '{pair['question']}' | Received ID: {ingested_id}")
    logging.info("Question-SQL training complete.")
...
if __name__ == '__main__':
    ...
    logging.info("Initializing Vanna for training...")
    with MyVanna(config=config) as vn:
        logging.info("Starting training process...")
        train_vanna(vn)
        logging.info("✅ Vanna model training finished successfully.")